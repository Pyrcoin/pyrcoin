...
//static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pyrcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pyrcoin Signed Message:\n";
...
{
    // Pyrcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // Pyrcoin: 840k blocks in ~4 years
...
//static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Pyrcoin: 3.5 days
//static const int64 nTargetSpacing = 2.5 * 60; // Pyrcoin: 2.5 minutes

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // Pyrcoin: 0.35 days
static const int64 nTargetSpacing = 15; // Pyrcoin: 15 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pyrcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pyrcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Pyrcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Pyrcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PyrcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PyrcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PyrcoinMiner(CWallet *pwallet)
{
    printf("PyrcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pyrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PyrcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PyrcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PyrcoinMiner, pwallet));
}
